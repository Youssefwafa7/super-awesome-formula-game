...
bool our::ShaderProgram::attach(const std::string &filename, GLenum type) const {
    ...
    if(!checkForShaderCompilationErrors(shader).empty()){
        std::cerr << "ERROR: Couldn't compile shader: " << filename << std::endl;
        return false;
    }
    glAttachShader(program, shader);
    glDeleteShader(shader); //The shader is already attached to the program, so we can delete it. Deleting the shader will not detach it from the program.
...
bool our::ShaderProgram::link() const {
    ...
    if(!checkForLinkingErrors(program).empty()){
